...
function generateMap(type) {
    const {cols, rows, tileSize} = type === "SMALL" ? CONFIG.SMALL_MAP : CONFIG.LARGE_MAP;
    ...
    // Clear spawn points (5x5 area to ensure walkability)
    ...
    while (placed < CONFIG.AMMO_PACK_COUNT) {
        ...
        if (grid[r][c] === 0) {
            ammo_pickups.push({
                id: `ammo_${placed}`,
                x: c * tileSize + tileSize / 2, // ← centered
                y: r * tileSize + tileSize / 2, // ← centered
