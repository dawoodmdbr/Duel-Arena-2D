...
function HomePage() {
  ...
  // Connect socket and start matchmaking
  const handlePlay = () => {
        ...
        const interval = setInterval(() => {
          setRemainingTime(prev => {
            if (prev <= 1) {
              ...
              socket.emit('find_match') // auto join when match ends
              return 0
            }
            return prev - 1
          })
  ...
  socket.on('room_updated', (data) => navigate('/game', { state: data }))
}
...
const handleCreateRoom = () => {
  ...
  socket.on('room_updated', (data) => { latestRoomData = data })
  socket.on('room_created', () => navigate('/game', { state: latestRoomData }))
